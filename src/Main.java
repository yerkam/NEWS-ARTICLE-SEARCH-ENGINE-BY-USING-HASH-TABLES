...
public class Main extends MainFunctionalities {
    public static void main(String[] args) throws IOException, InterruptedException{
        ...
        if( performansChoice == 1 ) performanceMatrix();
        ...
        while(!input.equals("9")){
            ...
            if(input.equals("1")){
                ...
                }// Şu an burada words arrayinde elemanlardan stop kelimeler boş ve aranacak kelimeler duruyor
                if(size == 0){ // Tüm kelimeler stop kelime ise
                    ...
                    LinkedList<String> relevantArticleIDs = new LinkedList<>(); // Alakalı makale ID'leri listesi
                    int maxFrequency = -1;
                    for(String key : keySet){ // Her makale ID'si için
                        int frequency = tempHashTable.get(key); // Makaledeki kelime frekansını al
                        if(frequency > maxFrequency){ 
                            maxFrequency = frequency; // En yüksek frekansı güncelle
                            relevantArticleIDs.clear(); // Önceki sonuçları temizle
                            relevantArticleIDs.add(key); // Yeni en alakalı makaleyi ekle
                        }else if(frequency == maxFrequency){
                            relevantArticleIDs.add(key); // Aynı frekans, listeye ekle
                        }
                    }
                    // En alakalı makaleyi yazdır
                    System.out.println("Found " + relevantArticleIDs.size() + " article(s) with frequency " + maxFrequency + ":");
                    for(String relevantArticleID : relevantArticleIDs){ // Her alakalı makale için
                    ...
                    for(int i = 0; i < words.length; i++){
                        String word = words[i];
                        if(word.equals("")) continue; // Stop kelime ise atla
                        HashTableInterface<String, Integer> tempHashTable = wordArticleCountMap.get(word);
                        if(tempHashTable == null) continue; // Kelime makalelerde yoksa atla
                    ...
                    for(String key : keySet){ // Her makale ID'si için
                        ...
                        if(frequency > maxFrequency){ // En yüksek frekansı güncelle
                            ...
                            relevantArticleIDs.clear();
                            relevantArticleIDs.add(key);
                        }else if(frequency == maxFrequency){
                            // Aynı frekans, listeye ekle
                            relevantArticleIDs.add(key);
                        }
                    }
                    // En alakalı makaleleri yazdır
                    System.out.println("Found " + relevantArticleIDs.size() + " article(s) with frequency " + maxFrequency + ":");
                    for(String relevantArticleID : relevantArticleIDs){ // Her alakalı makale için
                        LinkedList<String> Article = IDhashTable.get(relevantArticleID); // Makaleyi al
                        Article.pop(); // ID'yi kaldır ("" halde çıkıyor)
